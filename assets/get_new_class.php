...
class LTVW_Get_News_Class{
	...
	public static function html_news($url, $img_thumb, $arr_cat_fix=array(1)){
		...
		//Gate date	
		self::$date = self::$html->find('div[class="block_timer left txt_666"]',0)->plaintext;
		if(self::$date==""){
			self::$date = self::$html->find('div[class="block_timer left txt_666"]',1)->plaintext;
		}
	...
	public function html_list($link){
		...
		//}
		$i=0;
		foreach($html_link->find('.box_hot_news,.box_sub_hot_news .scroll-pane li, #col_1 .list_news li, #box_show_item_top .box_show_item_main,.box_show_item_sub .item_box_show') as $box){
			$html_link_sub = str_get_html($box);
			
			foreach($html_link_sub->find('.title_news a.txt_link,.block_news_big img, .block_image_news .thumb img, .box_show_item_title a, .txt_image img,.box_show_item_title a,.item_box_show img') as $box_sub){
	...
	public function get_news(){		
		...
		//Get title
		foreach(self::$html->find('div[class="title_news"] h1') as $title){
		...
		if(!$post_exists){								
			...
			foreach(self::$html->find('.short_intro') as $content_intro){
			...
			foreach(self::$html->find('div[class="fck_detail"], div[class="block_content_slide_showdetail"] #article_content') as $content){
				...
				$z=1;
				foreach(self::$html->find('div[class="fck_detail"] img, div[class="block_content_slide_showdetail"] #article_content img') as $img){
